...
// Cache location: <repo>/cursor/.bloxus-cache/
const REPO_ROOT = path.resolve(__dirname, "..");
const CACHE_DIR = path.join(REPO_ROOT, ".bloxus-cache");
...
function usage() {
 ...
 node scripts/api-dump.js class <Name> [--inherited] [--file PATH]
 node scripts/api-dump.js enum <Name> [--file PATH]
...
function loadDump(filePath) {
  ...
  if (!fs.existsSync(resolvedPath)) {
    fail(
      `API dump not found at ${resolvedPath}. Run "node cursor/scripts/api-dump.js fetch" first or pass --file PATH.`,
    );
  }
...
function collectMembers(classMap, classInfo, includeInherited) {
  if (!includeInherited) {
    return (classInfo.Members || []).map((member) => ({
      ...member,
...
function printClass(data, className, options) {
  ...
  for (const type of ["Property", "Function", "Event", "Callback"]) {
    ...
    for (const member of items) {
      const suffix =
        member.source === classInfo.Name ? "" : ` (from ${member.source})`;
...
function printEnum(data, enumName) {
  ...
  for (const item of enumInfo.Items || []) {
    const legacyNames =
      Array.isArray(item.LegacyNames) && item.LegacyNames.length > 0
...
function searchMembers(data, memberName, typeFilter) {
  ...
  for (const result of results.slice(0, 100)) {
    console.log(
      `- ${result.className}.${result.member.Name} [${result.member.MemberType}]`,
    );
  }
